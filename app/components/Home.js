...
import { SafeAreaView, View, StyleSheet, ScrollView, Image, Text, FlatList, TouchableOpacity, ImageBackground } from 'react-native';
...
const Home = ({ navigation }) => {
    ...
    const renderDisItem = ({ item }) => {
        return (
            <TouchableOpacity onPress={() => navigation.navigate('Details', { item: item })}>
                <ImageBackground source={item.imagesmall} style={[styles.dis_item, { marginLeft: item.id === 1 ? 20 : 0 }]} imageStyle={styles.dis_item_image}>
    ...
    const renderLearnItem = ({ item }) => {
        return (
            <ImageBackground source={item.image} style={[styles.learn_wrapper, { marginLeft: item.id === 1 ? 20 : 0 }]} imageStyle={styles.learn_image_wrapper}>
        ...
        <SafeAreaView style={styles.main}>
            <ScrollView>
                ...
                <View style={styles.dis_item_wrapper}>
                    <FlatList data={DiscoverData} renderItem={renderDisItem} keyExtractor={(item) => item.id} horizontal showsHorizontalScrollIndicator={false} />
                </View>

                <View>
                    <Text style={styles.act_title}>Activities</Text>
                    <FlatList data={ActivitiesData} renderItem={renderActItem} keyExtractor={(item) => item.id} horizontal showsHorizontalScrollIndicator={false} />
                </View>

                <View>
                    <Text style={styles.learn_title}>Learn more</Text>
                    <FlatList data={LearnMoreData} renderItem={renderLearnItem} keyExtractor={(item) => item.id} horizontal showsHorizontalScrollIndicator={false} />
                </View>
            </ScrollView>
        </SafeAreaView>
    )
